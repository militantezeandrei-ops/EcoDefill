 ...
 *   Edge Impulse Arduino export installed as bottles_inferencing
 */
...
static constexpr float BOTTLE_DECISION_THRESHOLD = 0.50f;
...
void tuneSensor() {
  ...
  // Flipped off (normal upright orientation)
  s->set_vflip(s, 0);
  s->set_hmirror(s, 0);
...
bool captureAndClassifyBottle(bool* detected) {
  ...
  *detected = false;
  ...
  for (uint32_t i = 0; i < result.bounding_boxes_count; ++i) {
    ...
    if (isRejectBottleLabel(bb.label)) {
      status = "REJECTED (invalid label)";
    } else if (bb.value < BOTTLE_DECISION_THRESHOLD) {
      status = "LOW CONFIDENCE (below threshold)";
    } else {
      status = "ACCEPTED";
      found_target = true;
      ++found_count;
    ...
    Serial.printf("[BOTTLE]   -> [%s] %s: %.2f%% [x=%u, y=%u, w=%u, h=%u]\n",
                  status,
                  bb.label,
                  bb.value * 100.0f,
                  bb.x,
  ...
  if (printed_count == 0) {
    Serial.println("[BOTTLE]   (no detections found in frame)");
  }
